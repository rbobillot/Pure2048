...
object Output {
  ...
  def displayGuiGrid(frame: JFrame, gameBoardActor: ActorRef): IO[Unit] =
    for {
      p <- IO.pure(new BoardPanel(frame, gameBoardActor)) // TODO: should be removed (for now it avoids the initial keypress bug)
